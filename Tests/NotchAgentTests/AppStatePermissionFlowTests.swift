...
final class AppStatePermissionFlowTests: XCTestCase {
    ...
    func testCodexAlwaysAllowPersistsRuleWithoutUnsupportedUpdatedPermissions() async throws {
        ...
        let rules = try readNotchAgentRules(in: codexHome)
        XCTAssertTrue(rules.contains(#"pattern = ["php", "vendor/bin/phpstan", "analyse"]"#))
        XCTAssertTrue(rules.contains(#"decision = "allow""#))
    }

    func testCodexAlwaysAllowDoesNotDuplicateExistingNotchAgentRule() throws {
        let codexHome = makeTemporaryCodexHome()
        defer { try? FileManager.default.removeItem(at: codexHome) }
        ...
        let contents = try readNotchAgentRules(in: codexHome)
    ...
    private func notchAgentRulesPath(in codexHome: URL) -> URL {
        codexHome
            .appendingPathComponent("rules", isDirectory: true)
            .appendingPathComponent("notchagent.rules")
    }

    private func readNotchAgentRules(in codexHome: URL) throws -> String {
        try String(contentsOf: notchAgentRulesPath(in: codexHome), encoding: .utf8)
    }
