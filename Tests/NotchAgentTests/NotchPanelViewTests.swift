...
final class NotchPanelViewTests: XCTestCase {
    ...
    func testIdleIndicatorDoesNotHardcodeClaudeMascot() throws {
        let sourceURL = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .appendingPathComponent("Sources/NotchAgent/NotchPanelView.swift")
        let source = try String(contentsOf: sourceURL, encoding: .utf8)

        XCTAssertFalse(
            source.contains(#"MascotView(source: "claude", status: .idle"#),
            "No-session idle indicator must use the configured default mascot, not hardcoded Claude."
        )
    }
