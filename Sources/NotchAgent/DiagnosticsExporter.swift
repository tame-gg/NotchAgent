...
struct DiagnosticsExporter {
    ...
    static func export() {
        ...
        panel.nameFieldStringValue = "NotchAgent-Diagnostics-\(timestamp()).zip"
    ...
    private static func buildArchive(saveTo destination: URL) throws -> URL {
        ...
        let tmp = fm.temporaryDirectory.appendingPathComponent("NotchAgent-Diag-\(UUID().uuidString)", isDirectory: true)
        let root = tmp.appendingPathComponent("NotchAgent-Diagnostics-\(timestamp())", isDirectory: true)
        ...
        let configs: [(source: String, dest: String)] = [
            ...
            ("\(home)/.notchagent/sessions.json", "configs/persisted-sessions.json"),
        ]
        for item in configs {
        ...
        let logOutput = runCommand("/usr/bin/log", args: [
            ...
            "--last", "2h", "--predicate", "subsystem == \"com.notchagent\""
        ])
    ...
    private static func copyCrashReports(to dir: URL) {
        ...
        let recent = files
            .filter { $0.lastPathComponent.lowercased().contains("notchagent") }
            .sorted {
