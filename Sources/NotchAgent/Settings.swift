...
enum SettingsKey {
    ...
    static func customMascotPath(_ source: String) -> String { "customMascotPath_\(source)" }
    ...
    // Panel mode: "notch" (default) or "floating"
    static let panelMode = "panelMode"
    static let floatingPositionX = "floatingPositionX"
    static let floatingPositionY = "floatingPositionY"
    ...
    static let approvalRules = "approvalRules"
    static let approvalPauseUntil = "approvalPauseUntil"
...
struct SettingsDefaults {
    ...
    static let panelMode = "notch"
    static let floatingPositionX = 0.0
    static let floatingPositionY = 0.0
    ...
    static let approvalRules = Data()
    static let approvalPauseUntil = 0.0
...
class SettingsManager {
    ...
    private init() {
        defaults.register(defaults: [
            ...
            SettingsKey.panelMode: SettingsDefaults.panelMode,
            SettingsKey.floatingPositionX: SettingsDefaults.floatingPositionX,
            SettingsKey.floatingPositionY: SettingsDefaults.floatingPositionY,
            SettingsKey.defaultSource: SettingsDefaults.defaultSource,
            SettingsKey.autoApproveTools: SettingsDefaults.autoApproveTools,
            SettingsKey.approvalRules: SettingsDefaults.approvalRules,
            SettingsKey.approvalPauseUntil: SettingsDefaults.approvalPauseUntil,
    ...
    var panelMode: String {
        get { defaults.string(forKey: SettingsKey.panelMode) ?? SettingsDefaults.panelMode }
        set { defaults.set(newValue, forKey: SettingsKey.panelMode) }
    }

    var floatingPosition: CGPoint {
        get {
            let x = defaults.double(forKey: SettingsKey.floatingPositionX)
            let y = defaults.double(forKey: SettingsKey.floatingPositionY)
            return CGPoint(x: x, y: y)
        }
        set {
            defaults.set(Double(newValue.x), forKey: SettingsKey.floatingPositionX)
            defaults.set(Double(newValue.y), forKey: SettingsKey.floatingPositionY)
        }
    }
    ...
    var approvalRules: [ApprovalRule] {
        get {
            guard let data = defaults.data(forKey: SettingsKey.approvalRules),
                  !data.isEmpty,
        ...
        set {
            ...
            defaults.set(data, forKey: SettingsKey.approvalRules)
        }
    }

    var approvalPauseUntil: Date? {
        get {
            let interval = defaults.double(forKey: SettingsKey.approvalPauseUntil)
            guard interval > Date().timeIntervalSince1970 else { return nil }
            return Date(timeIntervalSince1970: interval)
        }
        set {
            defaults.set(newValue?.timeIntervalSince1970 ?? 0, forKey: SettingsKey.approvalPauseUntil)
        }
    }

    var isApprovalPaused: Bool {
