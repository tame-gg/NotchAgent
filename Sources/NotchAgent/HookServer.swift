...
private let log = Logger(subsystem: "com.notchagent", category: "HookServer")
...
class HookServer {
    ...
    private static func forwardEventToWebhook(_ event: HookEvent) {
        ...
        request.setValue("NotchAgent-Webhook/1.0", forHTTPHeaderField: "User-Agent")
    ...
    static func routeKind(for event: HookEvent) -> RouteKind {
        // CLI command envelope: a minimal JSON with _notchagent_command overrides normal routing
        if event.rawJSON["_notchagent_command"] is String {
            return .command
        }
    ...
    private func processRequest(data: Data, connection: NWConnection) {
        ...
        case .event:
            ...
            sendResponse(connection: connection, data: Data("{}".utf8))
        case .command:
            if let command = event.rawJSON["_notchagent_command"] as? String {
                let response = appState.handleCommand(command)
                sendResponse(connection: connection, data: response)
            } else {
                sendResponse(connection: connection, data: Data(#"{"error":"missing_command"}"#.utf8))
            }
        }
    }
    ...
    /// NotchAgentBridge/main.swift), which produces an immediate EOF on the read side.
