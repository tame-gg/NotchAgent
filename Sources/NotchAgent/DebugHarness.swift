...
//   e.g.  .build/debug/NotchAgent --preview approval
//
// Scenarios:
...
enum DebugHarness {
    ...
    private static func applyAllCLI(to state: AppState) {
        ...
        s7.status = .running
        s7.cwd = "/tmp/demo-opencode"
        s7.model = "gpt-4.1"
        s7.source = "opencode"
        s7.currentTool = "Bash"
        s7.toolDescription = "npm test"
        s7.lastUserPrompt = "Run test suite"
        s7.addRecentMessage(ChatMessage(isUser: true, text: "Run test suite"))
        s7.termApp = "Ghostty"
    ...
    private static func applyStress(to state: AppState) {
        let sources = ["claude", "codex", "gemini", "cursor", "copilot", "qoder", "droid", "opencode", "kimi"]
