...
private let log = Logger(subsystem: "com.notchagent", category: "Panel")
...
class PanelWindowController: NSObject, NSWindowDelegate {
    ...
    private var lastPanelMode = SettingsDefaults.panelMode
    ...
    func showPanel() {
        ...
        let isFloating = SettingsManager.shared.panelMode == "floating"
    ...
    private func observeSettingsChanges() {
        ...
        lastPanelMode = SettingsManager.shared.panelMode
        let observer = NotificationCenter.default.addObserver(
            ...
            Task { @MainActor in
                ...
                let newPanelMode = SettingsManager.shared.panelMode
                if newChoice != self.lastDisplayChoice {
                    ...
                    self.refreshCurrentScreen(forceRebuild: true)
                } else if newPanelMode != self.lastPanelMode {
                    self.lastPanelMode = newPanelMode
                    self.refreshCurrentScreen(forceRebuild: true)
                } else {
                    self.updateVisibility()
                    self.updatePosition()
    ...
    private func panelFrame(for screen: NSScreen) -> NSRect {
        ...
        if SettingsManager.shared.panelMode == "floating" {
            let stored = SettingsManager.shared.floatingPosition
            let x = stored.x > 0 ? CGFloat(stored.x) : screenFrame.midX - size.width / 2
            let y = stored.y > 0 ? CGFloat(stored.y) : screenFrame.midY - size.height / 2
            return NSRect(
    ...
    private func setupHorizontalDragMonitor() {
        ...
        localDragMonitor = NSEvent.addLocalMonitorForEvents(matching: [.leftMouseDown, .leftMouseDragged, .leftMouseUp]) { [weak self] event in
            ...
            let isFloating = SettingsManager.shared.panelMode == "floating"
            guard isFloating || SettingsManager.shared.allowHorizontalDrag else { return event }
            ...
            case .leftMouseDown:
                if event.window === panel {
                    ...
                    self.dragStartMouseY = NSEvent.mouseLocation.y
                    self.dragStartPanelY = panel.frame.origin.y
            ...
            case .leftMouseDragged:
                if let startMouseX = self.dragStartMouseX,
                   let startPanelX = self.dragStartPanelX {
                    ...
                    if isFloating {
                        let deltaY = NSEvent.mouseLocation.y - (self.dragStartMouseY ?? NSEvent.mouseLocation.y)
                        let startY = self.dragStartPanelY ?? panel.frame.origin.y
                        let newY = min(max(startY + deltaY, screen.frame.minY), screen.frame.maxY - size.height)
            ...
            case .leftMouseUp:
                if self.isDraggingPanel, let panel = self.panel {
                    if isFloating {
                        SettingsManager.shared.floatingPosition = CGPoint(
