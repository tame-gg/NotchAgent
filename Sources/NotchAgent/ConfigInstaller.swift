...
struct ConfigInstaller {
    private static let notchagentDir = NSHomeDirectory() + "/.notchagent"
    private static let bridgePath = notchagentDir + "/notchagent-bridge"
    private static let hookScriptPath = notchagentDir + "/notchagent-hook.sh"
    private static let hookCommand = "~/.notchagent/notchagent-hook.sh"
    ...
    private static let bridgeCommand = notchagentDir + "/notchagent-bridge"
    ...
    private static let piExtensionPath = NSHomeDirectory() + "/.pi/agent/extensions/notchagent.ts"
    ...
    private static let ompExtensionPath = NSHomeDirectory() + "/.omp/agent/extensions/notchagent.ts"
    ...
    private static let builtInCLIs: [CLIConfig] = [
        ...
        CLIConfig(
            ...
            configPath: ".copilot/hooks/notchagent.json", configKey: "hooks",
            format: .copilot,
            events: [
                ("sessionStart", 5, false),
                ("sessionEnd", 5, true),
        ...
        // Kiro CLI — agent-scoped JSON at ~/.kiro/agents/notchagent.json.
        // User must launch with `kiro --agent notchagent` for hooks to fire (#127).
        CLIConfig(
            name: "Kiro", source: "kiro",
            configPath: ".kiro/agents/notchagent.json", configKey: "hooks",
            format: .kiroAgent,
        ...
        CLIConfig(
            name: "pi",
            source: "pi",
            configPath: ".pi/agent/extensions/notchagent.ts",
            configKey: "",
            format: .none,
            events: []
        )
        ...
        CLIConfig(
            name: "Oh My Pi",
            source: "omp",
            configPath: ".omp/agent/extensions/notchagent.ts",
            configKey: "",
            format: .none,
            events: []
        )
    ...
    private static let hookScript = """
        #!/bin/bash
        # NotchAgent hook v\(hookScriptVersion) — native bridge with shell fallback
        BRIDGE="$HOME/.notchagent/notchagent-bridge"
        if [ -x "$BRIDGE" ]; then
          exec "$BRIDGE" "$@"
        fi
        # Fallback: original shell approach (no binary installed yet)
        SOCK="/tmp/notchagent-$(id -u).sock"
        [ -S "$SOCK" ] || exit 0
        INPUT=$(cat)
    ...
    private static let opencodePluginPath = NSHomeDirectory() + "/.config/opencode/plugins/notchagent.js"
    ...
    static func install() -> Bool {
        ...
        // Ensure ~/.notchagent directory
        try? fm.createDirectory(atPath: notchagentDir, withIntermediateDirectories: true)
    ...
    static func installExternalHooks(cli: CLIConfig, fm: FileManager) -> Bool {
        ...
        if cli.format == .copilot, (originalText == nil || originalText?.isEmpty == true) {
            ...
            // `kiro --agent notchagent`.
            seeded = """
            {
              "name": "notchagent",
              "description": "Auto-generated by NotchAgent — relays Kiro hook events to the macOS Dynamic Island. Launch with `kiro --agent notchagent`."
            }
            """
        }
    ...
    private static func expectedTraecliCommandCandidates(source: String) -> [String] {
        ...
        let tilde = "~/.notchagent/notchagent-bridge --source \(source)"
        let tildeQuoted = "\"~/.notchagent/notchagent-bridge\" --source \(source)"
        ...
        contents = contents
            ...
            .map { line in
                ...
                if trimmed.hasPrefix("hooks =") {
                    return "# [NotchAgent] commented out legacy scalar hooks to avoid TOML conflict\n# \(line)"
                }
                return line
            }
            .joined(separator: "\n")
    ...
    static func removeKimiHooks(from contents: String) -> String {
        ...
        while i < lines.count {
            ...
            if line.trimmingCharacters(in: .whitespaces) == "[[hooks]]" {
                ...
                if !blockText.contains("notchagent-bridge") {
    ...
    static func contentsContainsKimiHook(_ contents: String, event: String) -> Bool {
        ...
        for line in lines {
            ...
            if inHookBlock {
                ...
                if currentEvent == event && trimmed.contains("notchagent-bridge") {
                    return true
                }
            }
        }
    ...
    internal static func uninstallHooks(cli: CLIConfig, fm: FileManager) {
        ...
        if cli.format == .kimi {
            ...
            for line in lines {
                let trimmed = line.trimmingCharacters(in: .whitespaces)
                if trimmed == "# [NotchAgent] commented out legacy scalar hooks to avoid TOML conflict" {
                    continue
                }
    ...
    private static let clineHookMarker = "notchagent-bridge --source cline"
    ...
    // status to NotchAgent, then immediately return {"cancel":false} to Cline.
    private static let clineHookScript = """
        #!/bin/bash
        INPUT=$(cat)
        printf '%s' "$INPUT" | ~/.notchagent/notchagent-bridge --source cline "$@" >/dev/null 2>&1 &
        printf '{"cancel":false}'
        """
        ...
        if fm.fileExists(atPath: hookScriptPath) {
            if let existing = fm.contents(atPath: hookScriptPath),
               let str = String(data: existing, encoding: .utf8) {
                ...
                let hasCurrentVersion = str.contains("# NotchAgent hook v\(hookScriptVersion)")
    ...
    private static func installBridgeBinary(fm: FileManager) {
        ...
        var srcPath = contentsDir + "/Helpers/notchagent-bridge"
        if !fm.fileExists(atPath: srcPath) { srcPath = execDir + "/notchagent-bridge" }
    ...
    private static func opencodePluginSource() -> String? {
        ...
        if let url = Bundle.appModule.url(forResource: "notchagent-opencode", withExtension: "js", subdirectory: "Resources"),
           let src = try? String(contentsOf: url) { return src }
        // Fallback: try without subdirectory
        if let url = Bundle.appModule.url(forResource: "notchagent-opencode", withExtension: "js"),
           let src = try? String(contentsOf: url) { return src }
        return nil
    }
    ...
    /// Current pi extension version — bump when notchagent-pi.ts changes.
    private static let piExtensionVersion = "v1"

    private static func piExtensionSource() -> String? {
        if let url = Bundle.appModule.url(forResource: "notchagent-pi", withExtension: "ts", subdirectory: "Resources"),
           let src = try? String(contentsOf: url) { return src }
        if let url = Bundle.appModule.url(forResource: "notchagent-pi", withExtension: "ts"),
           let src = try? String(contentsOf: url) { return src }
        return nil
    }
    ...
    static func uninstallPiExtension(
        ...
        guard fm.fileExists(atPath: piExtensionPath),
              ...
              let content = String(data: data, encoding: .utf8),
              content.contains("NotchAgent pi extension")
        else { return }
    ...
    static func isPiExtensionInstalled(
        ...
        return content.contains("NotchAgent pi extension")
            && content.contains("// version: \(piExtensionVersion)")
    }

    private static func ompExtensionSource() -> String? {
        if let url = Bundle.appModule.url(forResource: "notchagent-omp", withExtension: "ts", subdirectory: "Resources"),
           let src = try? String(contentsOf: url) { return src }
        if let url = Bundle.appModule.url(forResource: "notchagent-omp", withExtension: "ts"),
           let src = try? String(contentsOf: url) { return src }
        return nil
    }
    ...
    private static func backupOpencodeConfig(at path: String, original: String, fm: FileManager) {
        ...
        // Skip if any previous notchagent backup exists for this file.
        if let entries = try? fm.contentsOfDirectory(atPath: dir),
           entries.contains(where: { $0.hasPrefix(name + ".notchagent.bak.") }) {
            return
        }
        let formatter = ISO8601DateFormatter()
        formatter.formatOptions = [.withYear, .withMonth, .withDay, .withTime]
        let stamp = formatter.string(from: Date()).replacingOccurrences(of: ":", with: "")
        let backupPath = "\(path).notchagent.bak.\(stamp)"
        fm.createFile(atPath: backupPath, contents: Data(original.utf8))
    }

    /// Current OpenCode plugin version — bump when notchagent-opencode.js changes
