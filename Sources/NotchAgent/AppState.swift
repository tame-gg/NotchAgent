...
private let log = Logger(subsystem: "com.notchagent", category: "AppState")
...
final class AppState {
    ...
    private let maxTimelineEvents = 250

    var approvalRules: [ApprovalRule] {
        get { SettingsManager.shared.approvalRules }
        set { SettingsManager.shared.approvalRules = newValue }
    }

    var approvalPauseUntil: Date? {
        get { SettingsManager.shared.approvalPauseUntil }
        set { SettingsManager.shared.approvalPauseUntil = newValue }
    }

    var approvalPauseRemaining: TimeInterval {
        guard let until = approvalPauseUntil else { return 0 }
    ...
    func recordTimelineEvent(
        ...
        let project = session?.projectDisplayName ?? session?.cwd.map { URL(fileURLWithPath: $0).lastPathComponent } ?? "Unknown"
        let source = session?.source ?? "unknown"
        ...
        if timelineEvents.count > maxTimelineEvents {
            timelineEvents.removeLast(timelineEvents.count - maxTimelineEvents)
        }
    }

    func sessionMetrics() -> [SessionMetrics] {
        sessions.map { sessionId, session in
            let approvals = timelineEvents.filter {
                $0.sessionId == sessionId && $0.eventName == "PermissionRequest"
            }
            let denied = approvals.filter { $0.decision == "deny" }.count
            return SessionMetrics(
                ...
                deniedApprovalCount: denied,
                elapsedSeconds: max(0, session.lastActivity.timeIntervalSince(session.startTime)),
    ...
    func pauseApprovals(minutes: Int) {
        approvalPauseUntil = Date().addingTimeInterval(TimeInterval(max(1, minutes) * 60))
    }

    func resumeApprovals() {
        approvalPauseUntil = nil
        showNextPending()
        refreshDerivedState()
    }

    func matchedApprovalRule(for event: HookEvent) -> ApprovalRule? {
        approvalRules.first { $0.matches(event: event) }
    }

    private func responseData(for rule: ApprovalRule, event: HookEvent) -> Data {
        let behavior = rule.decision.rawValue
        let response = #"{"hookSpecificOutput":{"hookEventName":"PermissionRequest","decision":{"behavior":"\#(behavior)"}}}"#
        return Data(response.utf8)
    }

    private func statusRank(_ status: AgentStatus) -> Int {
        switch status {
        case .waitingApproval, .waitingQuestion: return 4
    ...
    func handleEvent(_ event: HookEvent) {
        ...
        // Update estimated cost on tool-use events
        updateEstimatedCost(sessionId: sessionId, event: event, eventName: normalizedEventName)
        ...
        recordTimelineEvent(
            ...
            title: timelineTitle(for: normalizedEventName, event: event),
            detail: event.toolDescription,
            toolName: event.toolName,
            risk: normalizedEventName == "PermissionRequest" ? ApprovalPreview.risk(for: event) : nil
        )
    ...
    private func timelineTitle(for eventName: String, event: HookEvent) -> String {
        switch eventName {
        ...
        case "PreToolUse": return "Tool started: \(event.toolName ?? "Unknown")"
        case "PostToolUse": return "Tool completed: \(event.toolName ?? "Unknown")"
        case "PostToolUseFailure": return "Tool failed: \(event.toolName ?? "Unknown")"
        case "PermissionRequest": return "Approval requested: \(event.toolName ?? "Unknown")"
        case "Notification": return "Notification"
        case "Stop": return "Reply completed"
    ...
    private func executeEffect(_ effect: SideEffect, sessionId: String) {
        ...
        case .removeSession(let sid):
            ...
            if let session = sessions[sid] {
                let duration = session.lastActivity.timeIntervalSince(session.startTime)
                let completed = session.status == .idle && session.totalToolCallCount > 0
                let approvals = timelineEvents.filter { $0.sessionId == sid && $0.eventName == "PermissionRequest" }
                PerformanceTracker.recordSession(
                    source: session.source,
                    duration: duration,
                    toolCalls: session.totalToolCallCount,
                    completed: completed,
                    approvals: approvals.count,
                    denied: approvals.filter { $0.decision == "deny" }.count
                )
            }
            removeSession(sid)
    ...
    private func updateEstimatedCost(sessionId: String, event: HookEvent, eventName: String) {
        guard var session = sessions[sessionId] else { return }
        let model = session.model ?? session.source
        switch eventName {
        case "PreToolUse":
            session.estimatedCost += CostEstimator.incrementalToolCost(model: model)
        case "UserPromptSubmit":
            session.estimatedCost += CostEstimator.estimateCost(
                model: model, toolCallCount: 0, promptCount: 1, replyCount: 0
            )
        case "Stop", "TaskRoundComplete":
            session.estimatedCost += CostEstimator.estimateCost(
                model: model, toolCallCount: 0, promptCount: 0, replyCount: 1
            )
        default:
            break
        }
    ...
    func handlePermissionRequest(_ event: HookEvent, continuation: CheckedContinuation<Data, Never>) {
        ...
        if let rule = matchedApprovalRule(for: event) {
            pendingToolUses.removeValue(forKey: event.toolUseId ?? "")
            let decision = rule.decision.rawValue
            continuation.resume(returning: responseData(for: rule, event: event))
            sessions[sessionId]?.status = rule.decision == .allow ? .running : .idle
            sessions[sessionId]?.currentTool = rule.decision == .allow ? sessions[sessionId]?.currentTool : nil
            sessions[sessionId]?.toolDescription = rule.decision == .allow ? sessions[sessionId]?.toolDescription : nil
            recordTimelineEvent(
                sessionId: sessionId,
                eventName: "PermissionRequest",
                title: "Approval \(decision) by rule: \(rule.name)",
                detail: request.commandSummary,
                toolName: event.toolName,
                risk: request.risk,
                decision: decision
            )
        ...
        recordTimelineEvent(
            sessionId: sessionId,
            eventName: "PermissionRequest",
            title: "Approval queued: \(event.toolName ?? "Unknown")",
            detail: request.commandSummary,
            toolName: event.toolName,
            risk: request.risk
        )

        if approvalPauseUntil != nil {
            refreshDerivedState()
            return
        }
    ...
    func approvePermission(always: Bool = false) {
        ...
        recordTimelineEvent(
            sessionId: sessionId,
            eventName: "PermissionRequest",
            title: always ? "Approval allowed always" : "Approval allowed",
            detail: pending.commandSummary,
            toolName: pending.event.toolName,
            risk: pending.risk,
            decision: "allow"
        )
    ...
    /// Handle a CLI command sent via the Unix socket from `notchagent-cli`.
    func handleCommand(_ command: String) -> Data {
        ...
        case "list":
            let sessionList = sessions.map { id, session in
                return [
                    "id": id,
                    "source": session.source,
                    "project": session.projectDisplayName,
                    ...
                    "cost": String(format: "%.4f", session.estimatedCost),
                ] as [String: Any]
            }
            let payload: [String: Any] = ["sessions": sessionList]
            return (try? JSONSerialization.data(withJSONObject: payload)) ?? Data("{}".utf8)
        case "toggle":
        ...
        case "collapse":
            ...
            return Data(#"{"ok":true}"#.utf8)
        default:
            return Data(#"{"error":"unknown_command"}"#.utf8)
        }
    }
    ...
    func showNextPending() -> Bool {
        if approvalPauseUntil == nil, let idx = nextVisiblePermissionIndex() {
